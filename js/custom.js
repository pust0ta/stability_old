...
jQuery(function($){
    ...
    /* checkouts */
    $('.language-switcher-locale-url').after('<ul class="checkouts-switcher"></ul>');
    $('.header-top-left select option').each(function(){
        ($(this).is(':selected')) ? current = 'current' : current = '';
        $('.checkouts-switcher').append('<li><a href="#" class="' + current + '">' + $(this).text() + '</a></li>');
    });
    $('body').on('click', '.checkouts-switcher a', function(){
        $('.language-switcher-locale-url a').each(function(){
            $(this).attr('href',$(this).attr('href').replace("?go","прошел"));
        });
        ind = $(this).parents('li').index();
        $('.header-top-left select option:eq(' + ind + ')').prop('selected', 'selected');
        $('.header-top-left form').submit();
        return false;
    });
    $('.language-switcher-locale-url a').each(function(){
        $(this).attr('href',$(this).attr('href') + '?go');
    });
